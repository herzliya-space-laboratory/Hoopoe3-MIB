...
# 2. Save the excel file by name: "TLM.csv" as a *comma delimited csv* file
...
def parameter_data(param_name,param_type,param_unit,param_little ="false"):
    ...
    if (param_name=="-" or param_name==""):
        return param_text
    if (param_unit=="" or param_unit=="-" or param_unit=="N"):
        param_text = "  <Parameter Name=\""+param_name+"\" Type=\""+param_type+"\" IsLittleEndian=\"" +param_little+ "\" />\n"
    else:
...
for row in range(0, len(data)):
    if(data[row][3]!=''):
        ...
        result += sst_data(data[row][5],data[row][3])
        param_name = data[row][6]
        param_type = data[row][7]
        param_unit = data[row][9]
